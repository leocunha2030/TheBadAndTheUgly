...
public class PlayerIK : MonoBehaviour
{
    ...
    void OnAnimatorIK(int layerIndex)
    {
        if (animator != null)
        {
            // Se o jogador está mirando (segurando o botão direito do mouse)
            if (Input.GetMouseButton(1))
            {
                // Configura o IK para a mão direita
                animator.SetIKPositionWeight(AvatarIKGoal.RightHand, ikWeight);
                animator.SetIKRotationWeight(AvatarIKGoal.RightHand, ikWeight);
                animator.SetIKPosition(AvatarIKGoal.RightHand, rightHandTarget.position);
                animator.SetIKRotation(AvatarIKGoal.RightHand, rightHandTarget.rotation);

                // Configura o IK para a mão esquerda (caso haja alvo)
                ...
                // Configura para olhar na direção do alvo
                animator.SetLookAtWeight(ikWeight);
                animator.SetLookAtPosition(aimTarget.position);
            }
            else
            {
                // Desativa o IK quando não estiver mirando
